...
2. Создать пользователя БД для применения миграций и прохождения автотестов:
    ...
    ```
3. Выдать доступ ко всем привилегиям сервисному пользователю миграции:
    ```
    GRANT ALL PRIVILEGES ON DATABASE <DB_NAME> TO <DB_USER>;
    -- Для следующего шага предварительно требуется подключиться к БД <DB_NAME>
...
* Добавление индексов ускорило выполнение запроса примерно в 7.5 раз;
* Основной вклад внес индекс orders_status_date_idx на таблице orders по фильтруемым полям. Вместо последовательного сканирования Parallel Seq Scan таблицы orders был применен метод Bitmap Index Scan, что позволило сократить стоимость узла доступа к orders с примерно 147k до примерно 69k;
* Другие индексы не используются для данного запроса, так как выбранный планировщиком метод соединения Parallel Hash Join не требует индексного доступа к полям. Выбор такого метода, скорее всего, обусловлен тем, что индекс не содержит всех необходимых данных и БД все равно потребуется обращение к таблице для получения данных о количестве.